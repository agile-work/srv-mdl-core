...
func LoadAllPages(r *http.Request) *moduleShared.Response {
	...
	schemaIDColumn := fmt.Sprintf("%s.schema_id", shared.TableCoreSchPages)
	condition := builder.Equal(schemaIDColumn, schemaID)

	return db.Load(r, &pages, "LoadAllPages", shared.TableCoreSchPages, condition)
}
...
func LoadPage(r *http.Request) *moduleShared.Response {
	...
	pageIDColumn := fmt.Sprintf("%s.id", shared.TableCoreSchPages)
	condition := builder.Equal(pageIDColumn, pageID)

	return db.Load(r, &page, "LoadPage", shared.TableCoreSchPages, condition)
}
...
func UpdatePage(r *http.Request) *moduleShared.Response {
	...
	return db.Update(r, &page, "UpdatePage", shared.TableCoreSchPages, condition)
}
...
func DeletePage(r *http.Request) *moduleShared.Response {
	...
	return db.Remove(r, "DeletePage", shared.TableCoreSchPages, condition)
}
